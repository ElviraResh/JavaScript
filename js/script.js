...
// пример несбалансированных скобок: (, ((), ), Длля проверки баланса, недостаточно кол-во, важен тк же порядок, в котором они идут. 
// 1) Если перед нами открывающий элемент, то заносим его в стек
// 2) Если закрывающий, то достаем из стека элемент (очевидно, последний добавленный) и смотрим, что он открывающий для данного закрывающего. если проверка провалилась, значит выражение не соответствует формату
...
/* const chekIsBalanced = expression => {
    const stack = []; // инициализация стека
    for (const symbol of expression) {
        if (symbol === '(') {
            stack.push(symbol);
        } else if (symbol === ')') {
            // если для закрывающего элемента не нашлось подходящего, значит баланса нет
            if (!stack.pop()) {
                return false;
            }
        }
...
// генерация строчек в цикле
//const lists = ['milk', 'butter'];

//Решение: 
// 1) создать переменную result и записать в нее ul
// 2) пройтись циклом по элементам коллекции и дописать в результирующую строку элемент li
// 3) длбавить в конец </ul> и вернуть result
/* const buildHtmlList = lists => {
    let result = '<ul>';
...
console.log(buildHtmlList(lists)); */

// правильнее формировать массив, который потом с помощью метода join превратит в строку
// подобный код легче отлаживать, проводить дополнительные манипуляции
/* const lists = ['milk', 'butter'];
const buildHtmlList = lists => {
    const parts = [];
    for (const item of lists) {
    ...
    // в качестве разделителя между значения используется то, что передано первым параметром
...
// написать функцию, которая генерирует html список определений ( теги <dl>, <dt>, <dd>) ивозвращает получившуюся строку
// при отсутствии элементов в массиве функция возвращает пустую строку

// список определиний следующего формата
/* const definitions = [
...
// результат: '<dl><dt>definition1</dt><dd>description1</dd><dt>definition2</dt><dd>description2</dd></dl>'

// Решить задачу через преобразование в массив 
// 1) метод split, который разделяет строку на части
// 2) обойти массив получившихся слов, и преобразовать каждую букву слова в верхний регистр 
